...
// `<details>...</details>` block forms (ADR-0011). Both lower to the same
// `SectionNode`. HTML support is a scoped exception to the otherwise-strict
// no-HTML rule and accepts only `<details>` / `<summary>` and the `open`
// attribute; any other attribute or tag falls through to the text path
// (markdown-it runs with `html: false`, so unrecognised HTML survives as
// literal text without a diagnostic).
...
// HTML form. The opening line must be exactly `<details>` or
// `<details open>`, optionally followed by an inline
// `<summary>Title</summary>` on the same line. Body content begins on the
// next line; the close must be a line on its own holding `</details>`.
// All-on-one-line forms are not recognised (they would force a different
// inner-parsing path).
//
// Captures:
...
function sectionBlock(
  ...
  // output between the open / close pair.
...
export function sectionsPlugin(md: MarkdownIt): void {
  ...
  // line is not absorbed into a preceding paragraph. The `alt` array makes
  // sections interruptible inside paragraph / blockquote / list. The two
  // rules are independent: each returns false on no-match, letting the
  // next rule try.
